...
#ifndef BAASSGCDAsyncSocketLoggingEnabled
#define BAASSGCDAsyncSocketLoggingEnabled 0
#endif

#if BAASSGCDAsyncSocketLoggingEnabled
...
#define LogAsync   YES
#define LogContext BAASSGCDAsyncSocketLoggingContext
...
#ifndef BAASSGCDAsyncSocketLogLevel
#define BAASSGCDAsyncSocketLogLevel LOG_LEVEL_VERBOSE
#endif

// Log levels : off, error, warn, info, verbose
static const int logLevel = BAASSGCDAsyncSocketLogLevel;
...
NSString *const BAASSGCDAsyncSocketException = @"BAASSGCDAsyncSocketException";
NSString *const BAASSGCDAsyncSocketErrorDomain = @"BAASSGCDAsyncSocketErrorDomain";

NSString *const BAASSGCDAsyncSocketQueueName = @"BAASSGCDAsyncSocket";
NSString *const BAASSGCDAsyncSocketThreadName = @"BAASSGCDAsyncSocket-CFStream";

NSString *const BAASSGCDAsyncSocketManuallyEvaluateTrust = @"BAASSGCDAsyncSocketManuallyEvaluateTrust";
#if TARGET_OS_IPHONE
NSString *const BAASSGCDAsyncSocketUseCFStreamForTLS = @"BAASSGCDAsyncSocketUseCFStreamForTLS";
#endif
NSString *const BAASSGCDAsyncSocketSSLPeerID = @"BAASSGCDAsyncSocketSSLPeerID";
NSString *const BAASSGCDAsyncSocketSSLProtocolVersionMin = @"BAASSGCDAsyncSocketSSLProtocolVersionMin";
NSString *const BAASSGCDAsyncSocketSSLProtocolVersionMax = @"BAASSGCDAsyncSocketSSLProtocolVersionMax";
NSString *const BAASSGCDAsyncSocketSSLSessionOptionFalseStart = @"BAASSGCDAsyncSocketSSLSessionOptionFalseStart";
NSString *const BAASSGCDAsyncSocketSSLSessionOptionSendOneByteRecord = @"BAASSGCDAsyncSocketSSLSessionOptionSendOneByteRecord";
NSString *const BAASSGCDAsyncSocketSSLCipherSuites = @"BAASSGCDAsyncSocketSSLCipherSuites";
#if !TARGET_OS_IPHONE
NSString *const BAASSGCDAsyncSocketSSLDiffieHellmanParameters = @"BAASSGCDAsyncSocketSSLDiffieHellmanParameters";
#endif

enum BAASSGCDAsyncSocketFlags
{
...
enum BAASSGCDAsyncSocketConfig
{
...
@interface BAASSGCDAsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation BAASSGCDAsyncSocketPreBuffer
...
/**
 * The BAASSGCDAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface BAASSGCDAsyncReadPacket : NSObject
{
  @public
...
- (NSUInteger)readLengthForTermWithPreBuffer:(BAASSGCDAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;

- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes;

@end

@implementation BAASSGCDAsyncReadPacket
...
/**
 * The BAASSGCDAsyncWritePacket encompasses the instructions for any given write.
**/
@interface BAASSGCDAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation BAASSGCDAsyncWritePacket
...
/**
 * The BAASSGCDAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
**/
@interface BAASSGCDAsyncSpecialPacket : NSObject
{
  @public
...
@implementation BAASSGCDAsyncSpecialPacket
...
@implementation BAASSGCDAsyncSocket
{
	uint32_t flags;
	uint16_t config;
	
	__weak id<BAASSGCDAsyncSocketDelegate> delegate;
	...
	BAASSGCDAsyncReadPacket *currentRead;
	BAASSGCDAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable;
	
	BAASSGCDAsyncSocketPreBuffer *preBuffer;
	...
	BAASSGCDAsyncSocketPreBuffer *sslPreBuffer;
...
- (instancetype)initWithDelegate:(id<BAASSGCDAsyncSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq
{
	return [self initWithDelegate:aDelegate delegateQueue:dq socketQueue:NULL];
}

- (instancetype)initWithDelegate:(id<BAASSGCDAsyncSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		delegate = aDelegate;
		...
		{
			socketQueue = dispatch_queue_create([BAASSGCDAsyncSocketQueueName UTF8String], NULL);
		}
		...
		preBuffer = [[BAASSGCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<BAASSGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq error:(NSError**)error {
  return [self socketFromConnectedSocketFD:socketFD delegate:aDelegate delegateQueue:dq socketQueue:NULL error:error];
}

+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<BAASSGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError* __autoreleasing *)error
{
  __block BOOL errorOccured = NO;
  
  BAASSGCDAsyncSocket *socket = [[[self class] alloc] initWithDelegate:aDelegate delegateQueue:dq socketQueue:sq];
  ...
  dispatch_sync(socket->socketQueue, ^{ @autoreleasepool {
    ...
    if (retVal)
    {
      NSString *errMsg = NSLocalizedStringWithDefaultValue(@"BAASSGCDAsyncSocketOtherError",
                                                           @"BAASSGCDAsyncSocket", [NSBundle mainBundle],
      ...
      errorOccured = YES;
      if (error)
        *error = [NSError errorWithDomain:BAASSGCDAsyncSocketErrorDomain code:BAASSGCDAsyncSocketOtherError userInfo:userInfo];
      return;
    }
...
- (void)setDelegate:(id<BAASSGCDAsyncSocketDelegate>)newDelegate
{
	[self setDelegate:newDelegate synchronously:NO];
}

- (void)synchronouslySetDelegate:(id<BAASSGCDAsyncSocketDelegate>)newDelegate
{
...
- (void)getDelegate:(id<BAASSGCDAsyncSocketDelegate> *)delegatePtr delegateQueue:(dispatch_queue_t *)delegateQueuePtr
{
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
...
- (void)setDelegate:(id<BAASSGCDAsyncSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
	[self setDelegate:newDelegate delegateQueue:newDelegateQueue synchronously:NO];
}

- (void)synchronouslySetDelegate:(id<BAASSGCDAsyncSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (enableIPv4)
		{
			...
			__weak BAASSGCDAsyncSocket *weakSelf = self;
			
            dispatch_source_set_event_handler(self->accept4Source, ^{ @autoreleasepool {
			#pragma clang diagnostic push
			#pragma clang diagnostic warning "-Wimplicit-retain-self"
				
				__strong BAASSGCDAsyncSocket *strongSelf = weakSelf;
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		__strong id<BAASSGCDAsyncSocketDelegate> theDelegate = delegate;
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			// Create BAASSGCDAsyncSocket instance for accepted socket
			
			BAASSGCDAsyncSocket *acceptedSocket = [[[self class] alloc] initWithDelegate:theDelegate
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:BAASSGCDAsyncSocketErrorDomain code:BAASSGCDAsyncSocketBadConfigError userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:BAASSGCDAsyncSocketErrorDomain code:BAASSGCDAsyncSocketBadParamError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"BAASSGCDAsyncSocketConnectTimeoutError",
	                                                     @"BAASSGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Attempt to connect to host timed out", nil);
	
	NSDictionary *userInfo = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:BAASSGCDAsyncSocketErrorDomain code:BAASSGCDAsyncSocketConnectTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"BAASSGCDAsyncSocketReadMaxedOutError",
														 @"BAASSGCDAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:BAASSGCDAsyncSocketErrorDomain code:BAASSGCDAsyncSocketReadMaxedOutError userInfo:info];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"BAASSGCDAsyncSocketReadTimeoutError",
	                                                     @"BAASSGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Read operation timed out", nil);
	
	NSDictionary *userInfo = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:BAASSGCDAsyncSocketErrorDomain code:BAASSGCDAsyncSocketReadTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"BAASSGCDAsyncSocketWriteTimeoutError",
	                                                     @"BAASSGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Write operation timed out", nil);
	
	NSDictionary *userInfo = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:BAASSGCDAsyncSocketErrorDomain code:BAASSGCDAsyncSocketWriteTimeoutError userInfo:userInfo];
}

- (NSError *)connectionClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"BAASSGCDAsyncSocketClosedError",
	                                                     @"BAASSGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Socket closed by remote peer", nil);
	
	NSDictionary *userInfo = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:BAASSGCDAsyncSocketErrorDomain code:BAASSGCDAsyncSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:BAASSGCDAsyncSocketErrorDomain code:BAASSGCDAsyncSocketOtherError userInfo:userInfo];
}
...
- (BOOL)usingCFStreamForTLS
{
	...
	{
		// The startTLS method was given the BAASSGCDAsyncSocketUseCFStreamForTLS flag.
                        ...
                        tag:(long)tag
{
	...
	BAASSGCDAsyncReadPacket *packet = [[BAASSGCDAsyncReadPacket alloc] initWithData:buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
        ...
        if (!self->currentRead || ![self->currentRead isKindOfClass:[BAASSGCDAsyncReadPacket class]])
		{
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			if ([currentRead isKindOfClass:[BAASSGCDAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued BAASSGCDAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued BAASSGCDAsyncReadPacket");
...
- (void)doReadData
{
	...
	if ([self usingCFStreamForTLS])
	{
		...
		// Requested CFStream, rather than SecureTransport, for TLS (via BAASSGCDAsyncSocketUseCFStreamForTLS)
...
- (void)completeCurrentRead
{
	...
	{
		BAASSGCDAsyncReadPacket *theRead = currentRead; // Ensure currentRead retained since result may not own buffer
...
- (void)doReadTimeout
{
	...
	{
		BAASSGCDAsyncReadPacket *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	BAASSGCDAsyncWritePacket *packet = [[BAASSGCDAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
        ...
        if (!self->currentWrite || ![self->currentWrite isKindOfClass:[BAASSGCDAsyncWritePacket class]])
		{
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
			...
			if ([currentWrite isKindOfClass:[BAASSGCDAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued BAASSGCDAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued BAASSGCDAsyncWritePacket");
...
- (void)doWriteData
{
	...
	// Note: This method is not called if currentWrite is a BAASSGCDAsyncSpecialPacket (startTLS packet)
...
- (void)doWriteTimeout
{
	...
	{
		BAASSGCDAsyncWritePacket *theWrite = currentWrite;
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	BAASSGCDAsyncSpecialPacket *packet = [[BAASSGCDAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		#if TARGET_OS_IPHONE
		{
			BAASSGCDAsyncSpecialPacket *tlsPacket = (BAASSGCDAsyncSpecialPacket *)currentRead;
            NSDictionary *tlsSettings = @{};
            if (tlsPacket) {
                tlsSettings = tlsPacket->tlsSettings;
            }
			NSNumber *value = [tlsSettings objectForKey:BAASSGCDAsyncSocketUseCFStreamForTLS];
...
{
	BAASSGCDAsyncSocket *asyncSocket = (__bridge BAASSGCDAsyncSocket *)connection;
...
- (void)ssl_startTLS
{
	...
	NSNumber *shouldManuallyEvaluateTrust = [tlsSettings objectForKey:BAASSGCDAsyncSocketManuallyEvaluateTrust];
	...
	//  3. BAASSGCDAsyncSocketSSLPeerID
	//  4. BAASSGCDAsyncSocketSSLProtocolVersionMin
	//  5. BAASSGCDAsyncSocketSSLProtocolVersionMax
	//  6. BAASSGCDAsyncSocketSSLSessionOptionFalseStart
	//  7. BAASSGCDAsyncSocketSSLSessionOptionSendOneByteRecord
	//  8. BAASSGCDAsyncSocketSSLCipherSuites
	//  9. BAASSGCDAsyncSocketSSLDiffieHellmanParameters (Mac)
	//
	...
	value = [tlsSettings objectForKey:BAASSGCDAsyncSocketSSLPeerID];
	...
	{
		NSAssert(NO, @"Invalid value for BAASSGCDAsyncSocketSSLPeerID. Value must be of type NSData."
		             @" (You can convert strings to data using a method like"
		             @" [string dataUsingEncoding:NSUTF8StringEncoding])");
		
		[self closeWithError:[self otherError:@"Invalid value for BAASSGCDAsyncSocketSSLPeerID."]];
		return;
	}
	
	// 4. BAASSGCDAsyncSocketSSLProtocolVersionMin
	
	value = [tlsSettings objectForKey:BAASSGCDAsyncSocketSSLProtocolVersionMin];
	...
	{
		NSAssert(NO, @"Invalid value for BAASSGCDAsyncSocketSSLProtocolVersionMin. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for BAASSGCDAsyncSocketSSLProtocolVersionMin."]];
		return;
	}
	
	// 5. BAASSGCDAsyncSocketSSLProtocolVersionMax
	
	value = [tlsSettings objectForKey:BAASSGCDAsyncSocketSSLProtocolVersionMax];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for BAASSGCDAsyncSocketSSLProtocolVersionMax. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for BAASSGCDAsyncSocketSSLProtocolVersionMax."]];
		return;
	}
	
	// 6. BAASSGCDAsyncSocketSSLSessionOptionFalseStart
	
	value = [tlsSettings objectForKey:BAASSGCDAsyncSocketSSLSessionOptionFalseStart];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for BAASSGCDAsyncSocketSSLSessionOptionFalseStart. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for BAASSGCDAsyncSocketSSLSessionOptionFalseStart."]];
		return;
	}
	
	// 7. BAASSGCDAsyncSocketSSLSessionOptionSendOneByteRecord
	
	value = [tlsSettings objectForKey:BAASSGCDAsyncSocketSSLSessionOptionSendOneByteRecord];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for BAASSGCDAsyncSocketSSLSessionOptionSendOneByteRecord."
		             @" Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for BAASSGCDAsyncSocketSSLSessionOptionSendOneByteRecord."]];
		return;
	}
	
	// 8. BAASSGCDAsyncSocketSSLCipherSuites
	
	value = [tlsSettings objectForKey:BAASSGCDAsyncSocketSSLCipherSuites];
	if ([value isKindOfClass:[NSArray class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for BAASSGCDAsyncSocketSSLCipherSuites. Value must be of type NSArray.");
		
		[self closeWithError:[self otherError:@"Invalid value for BAASSGCDAsyncSocketSSLCipherSuites."]];
		return;
	}
	
	// 9. BAASSGCDAsyncSocketSSLDiffieHellmanParameters
	
	#if !TARGET_OS_IPHONE
	value = [tlsSettings objectForKey:BAASSGCDAsyncSocketSSLDiffieHellmanParameters];
	if ([value isKindOfClass:[NSData class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for BAASSGCDAsyncSocketSSLDiffieHellmanParameters. Value must be of type NSData.");
		
		[self closeWithError:[self otherError:@"Invalid value for BAASSGCDAsyncSocketSSLDiffieHellmanParameters."]];
		return;
	}
	#endif
	...
	if (value)
	{
		NSAssert(NO, @"Security option unavailable - kCFStreamSSLLevel"
		             @" - You must use BAASSGCDAsyncSocketSSLProtocolVersionMin & BAASSGCDAsyncSocketSSLProtocolVersionMax");
	...
	sslPreBuffer = [[BAASSGCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)ssl_continueSSLHandshake
{
	...
	else if (status == errSSLPeerAuthCompleted)
	{
		...
		else
		{
			...
			NSString *msg = @"BAASSGCDAsyncSocketManuallyEvaluateTrust specified in tlsSettings,"
...
- (void)cf_startTLS
{
	...
	NSAssert([currentRead isKindOfClass:[BAASSGCDAsyncSpecialPacket class]], @"Invalid read packet for startTLS");
	NSAssert([currentWrite isKindOfClass:[BAASSGCDAsyncSpecialPacket class]], @"Invalid write packet for startTLS");
...
+ (void)startCFStreamThreadIfNeeded
{
	...
	dispatch_once(&predicate, ^{
		...
		cfstreamThreadSetupQueue = dispatch_queue_create("BAASSGCDAsyncSocket-CFStreamThreadSetup", DISPATCH_QUEUE_SERIAL);
	});
...
{
	[[NSThread currentThread] setName:BAASSGCDAsyncSocketThreadName];
...
+ (void)scheduleCFStreams:(BAASSGCDAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(BAASSGCDAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	BAASSGCDAsyncSocket *asyncSocket = (__bridge BAASSGCDAsyncSocket *)pInfo;
